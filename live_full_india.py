...
async def change_stop_order_price(shortlist_option,option_type):
    ...
    ord1=StopOrder('BUY',quantity,sp-20)
    ...
    updat_order_csv(cont.localSymbol,df.loc[cont.localSymbol,'price'],'BUY','UPDATE',sp-20)
    print(p1)
    return shortlist_option
...
async def stop_order_on_leg(shortlist_option,df):
    ...
    p=df.loc[cont.localSymbol,'price']
    print(p)
    ord1=StopOrder('BUY',quantity,p+20)
    p1=ib.placeOrder(cont,ord1)
    updat_order_csv(cont.localSymbol,df.loc[cont.localSymbol,'price'],'BUY','STP',p+20)
    print(p1)
    shortlist_option['short_call_option']['stop_price']=p+20
    ...
    updat_order_csv(cont.localSymbol,df.loc[cont.localSymbol,'price'],'BUY','STP',p+20)
    print(p1)
    shortlist_option['short_put_option']['stop_price']=p+20
...
async def main():
    ...
    while True:
        ...
        #     if shortlist_option['short_call_option'].get('buy_price')+10<df.loc[shortlist_option['short_call_option'].get('name'),'price']:
        #         shortlist_option=await manage_iron_condor(shortlist_option,'short_put_option',shortlist_option['short_call_option'].get('buy_price')+10)
        #         first_trade_flag=2
        #         shortlist_option['first_trade_flag']=2
        #         store(shortlist_option)
        #     if shortlist_option['short_put_option'].get('buy_price')+10<df.loc[shortlist_option['short_put_option'].get('name'),'price']:
        #         shortlist_option=await manage_iron_condor(shortlist_option,'short_call_option',shortlist_option['short_put_option'].get('buy_price')+10)
        #         first_trade_flag=2
        #         shortlist_option['first_trade_flag']=2
        #         store(shortlist_option)
