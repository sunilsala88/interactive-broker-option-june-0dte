...
ib.connect("127.0.0.1", 7497, clientId=22)  # Replace with your connection details
...
short_price=0.10
long_price=0.02
start_hour,start_min=18,1
end_hour,end_min=23,1
all_option_contract={}
shortlist_option={}
...
file_path = os.getcwd() + '/order_filled_list1.csv'
...
async def manage_iron_condor(shortlist_option,option_leg,new_price):
    ...
    new_strke=new_option.strike
    old_strike=contract2.strike
    ...
    if leg_name in pos_df['name'].to_list() :
        ...
        if (abs(new_strke-old_strike)<3):
            ...
            print(pd1)
            shortlist_option[option_leg]['exist']=True
        else:
            shortlist_option[option_leg]['exist']=False
...
async def stop_order_on_leg(shortlist_option,df):
    ...
    if shortlist_option['short_call_option']['exist']:
...
async def main():
    ...
    while True:
        ...
        #         shortlist_option['short_call_option']['exist']=True
        #         first_trade_flag=2
        ...
        #         first_trade_flag=2
        #         shortlist_option['short_put_option']['exist']=True
        ...
        #         print('first trade 3')
        #     #change option leg stop order if other leg gets hit

        #         if shortlist_option['short_call_option']['exist']:
        #             if df.loc[shortlist_option['short_call_option'].get('name'),'price']>shortlist_option['short_call_option']['stop_price']:
        #                 if shortlist_option['short_put_option']['exist']:
        #                     shortlist_option=await change_stop_order_price(shortlist_option,'short_put_option')
        #                     first_trade_flag=4
                
        #         if shortlist_option['short_put_option']['exist']:
        #             if df.loc[shortlist_option['short_put_option'].get('name'),'price']>shortlist_option['short_put_option']['stop_price']:
        #                 if shortlist_option['short_call_option']['exist']:
        #                     shortlist_option=await change_stop_order_price(shortlist_option,'short_call_option')
        #                     first_trade_flag=4

        #     # if df.loc[shortlist_option['short_call_option'].get('name'),'price']>shortlist_option['short_call_option']['buy_price']+0.06:
        #     #     shortlist_option=await change_stop_order_price(shortlist_option,'short_put_option')
        #     #     first_trade_flag=4

        #     # if df.loc[shortlist_option['short_put_option'].get('name'),'price']>shortlist_option['short_put_option']['buy_price']+0.06:
        #     #     shortlist_option=await change_stop_order_price(shortlist_option,'short_call_option')
        #     #     first_trade_flag=4     
